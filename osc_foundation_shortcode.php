...
add_action('init','is_oscitas_theme_efs_exists');
function is_oscitas_theme_efs_exists(){
...
function osc_add_admin_efs_scripts() {
    ...
    if ('post-new.php' == $pagenow || 'post.php' == $pagenow) {
        ...
        //wp_enqueue_script('jquery.dialog', plugins_url('/js/jquery-ui-1.10.3.custom.min.js', __FILE__));
        //wp_enqueue_style('jquery.dialog.css', plugins_url('/styles/overcast/jquery-ui-1.10.3.custom.min.css', __FILE__));

        if (!get_option('EFS_CUSTOM_FOUNDATION_ICON_CSS', '')) {
            wp_enqueue_style('foundation-icons', plugins_url('/styles/foundation-icons.css', __FILE__));
        }
        if (!get_option('EFS_CUSTOM_FOUNDATION_ADMIN_CSS', '')) {
...
function osc_add_frontend_efs_scripts() {
    ...
    if (!$isSet) {
        ...
        if ($js == 1) {
            wp_enqueue_script('foundation-custom', plugins_url('/js/vendor/custom.modernizr.js', __FILE__), '', '', true);
            wp_enqueue_script('foundation', plugins_url('/js/foundation.min.js', __FILE__),array('jquery','foundation-custom'), '', true);
